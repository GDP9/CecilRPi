...
//		instructions.add(new Instruction("ypull", "Pulls a copy of the number on  the top of the stack into the Y-register", 15));
		 ...
		 * Binary instructions
		 */
		instructions.add(new Instruction("load", "[Opcode:18] Replaces the contents of the Accumulator with the integer stored at the labelled address", 18));
		...
		instructions.add(new Instruction("add", "[Opcode:20] Adds the integer stored at the labelled address to the Accumulator value", 20));
		instructions.add(new Instruction("sub", "[Opcode:21] Subtracts the value stored at the labelled address from the current Accumulator value", 21));
		...
		instructions.add(new Instruction("jump", "[Opcode:25] Passes control to the instruction beginning at the labelled address", 25));
		...
		instructions.add(new Instruction("jineg", "[Opcode:27] Jumps to the instruction at the labelled address if the negative flag is set", 27));
		instructions.add(new Instruction("jipos", "[Opcode:28] Jumps to the instruction at the labelled address if neither the zero or negative flags are set", 28));
		instructions.add(new Instruction("jizero", "[Opcode:29] Jumps to the instruction at the labelled address if the zero flag is set", 29));
		//instructions.add(new Instruction("jmptosr", "Jumps to the subroutuive starting at the given labelled address", 30));
		instructions.add(new Instruction("jicarry", "[Opcode:31] Jumps to the instruction at the labelled address if the carry flag is set", 31));
